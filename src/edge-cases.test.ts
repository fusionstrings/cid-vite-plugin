...
Deno.test({
    ...
    `);
            await Deno.writeTextFile(path.join(tempDir, 'a.js'), `console.log('same');`);
            await Deno.writeTextFile(path.join(tempDir, 'b.js'), `console.log('same');`);
...
Deno.test({
    ...
    `);
            ...
            const pngData = Uint8Array.from(atob('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg=='), c => c.charCodeAt(0));
